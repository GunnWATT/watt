...
export default function Event(props: GCalEvent) {
    ...
    if (!description) return (
        <li className="w-full text-left px-4 py-2 rounded bg-[color:var(--content-secondary)] text-sm bg-opacity-50">
    ...
    return (
        <Disclosure as={'li'}>
            {({open}) => (<>
                <Disclosure.Button className="flex items-center justify-between gap-2 w-full text-left px-4 py-2 rounded bg-[color:var(--content-secondary)] text-sm bg-opacity-50">
                    <div>
                        <strong>{summary}</strong>
                        <p className="secondary">{formatDateTime(start, end)}</p>
                        {location && <p className="secondary">@ {location}</p>}
